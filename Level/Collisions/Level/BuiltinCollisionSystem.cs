...
namespace PVZEngine.Collisions.Level
{
    public class BuiltinCollisionSystem : ICollisionSystem
    {
        ...
        public SerializableBuiltinCollisionSystem ToSerializable()
        {
            ...
            var seriEntities = new SerializableBuiltinCollisionSystemEntity[entities.Count];
            int entIndex = 0;
            foreach (var entPair in entities)
            {
                seriEntities[entIndex] = entPair.Value.ToSerializable();
                entIndex++;
            }
            seri.entities = seriEntities;

            var seriEntityTrash = new SerializableBuiltinCollisionSystemEntity[entities.Count];
            entIndex = 0;
            foreach (var entPair in entityTrash)
            {
                seriEntityTrash[entIndex] = entPair.Value.ToSerializable();
                entIndex++;
            }
            seri.entityTrash = seriEntityTrash;
            return seri;
        }
