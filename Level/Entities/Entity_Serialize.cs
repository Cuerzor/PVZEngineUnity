...
namespace PVZEngine.Entities
{
    public sealed partial class Entity
    {
        ...
        public SerializableEntity ToSerializable()
        {
            ...
            seri.takenConveyorSeeds = new Dictionary<string, int>();
            foreach (var pair in takenConveyorSeeds)
            {
                seri.takenConveyorSeeds.Add(pair.Key.ToString(), pair.Value);
            }
        ...
        private void InitFromSerializable(SerializableEntity seri)
        {
            ...
            takenConveyorSeeds = new Dictionary<NamespaceID, int>();
            if (seri.takenConveyorSeeds != null)
            {
                foreach (var pair in seri.takenConveyorSeeds)
                {
                    takenConveyorSeeds.Add(NamespaceID.ParseStrict(pair.Key), pair.Value);
                }
            }
        ...
        private void LoadEntityReferencesFromSerializable(SerializableEntity seri)
        {
            ...
            {
                foreach (var seriChild in seri.children)
                {
                    var child = Level.FindEntityByID(seriChild);
                    if (child == null)
                        continue;
                    children.Add(child);
                }
