...
namespace PVZEngine.Level
{
    public partial class LevelEngine
    {
        ...
        {
            var seriComponents = new Dictionary<string, ISerializableLevelComponent>();
            foreach (var component in levelComponents)
            {
                seriComponents.Add(component.GetID().ToString(), component.ToSerializable());
            }
        ...
        {
            seri.energy = Energy;
            seri.delayedEnergyEntities = new SerializableDelayedEnergy[delayedEnergyEntities.Count];
            int index = 0;
            foreach (var pair in delayedEnergyEntities)
            {
                seri.delayedEnergyEntities[index] = new SerializableDelayedEnergy() { entityId = pair.Key.ID, energy = pair.Value };
                index++;
            }
        }
