...
namespace PVZEngine.Modifiers
{
    ...
    {
        public override Color CalculateGeneric(Color value, IList<ModifierSourceItem> modifiers)
        {
            if (modifiers.Count == 0)
                return value;

            using var modifiersBufferItem = ListPool<ModifierSourceItem>.Rent();
            var modifiersBuffer = modifiersBufferItem.Value;

            modifiersBuffer.AddRange(modifiers);
            modifiersBuffer.Sort(ModifierSourceItemComparer.Default);
            foreach (var item in modifiersBuffer)
            {
